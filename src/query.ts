...
function buildQueryAgents(sourceFiles: string[], save: boolean, wikiContent: string): string {
  ...
  const lines = [
    ...
    ``,
    `To get the bbox: read the .json file in .llm-kb/wiki/sources/ (same name as the .md file but .json).`,
    `Use bash + node to search the .json textItems on the cited page. Example:`,
    `  node -e "const d=require('./.llm-kb/wiki/sources/FILE.json'); const p=d.pages.find(p=>p.page===PAGE); p.textItems.filter(t=>t.text.includes('KEYWORD')).forEach(t=>console.log(JSON.stringify(t)))"`,
    `Then compute the merged bbox from the matching textItems.`,
